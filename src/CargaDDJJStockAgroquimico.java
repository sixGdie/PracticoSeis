...
public class CargaDDJJStockAgroquimico {
	...
	private static String[][] agroquimicos = {{"1001", "2EC JEBAGRO", "A"}, {"1002", "D 60 GREEN CROPS", "V"}, {"1003", "ESTER 97", "A"}, {"1004", "AMINA ICONA SL", "A"}, {"1005", "ABAMECTINA AGRO", "A"}, {"1006", "FURACARB 31 TS", "R"}, {"1007", "FURAFARM 48 F", "R"}, {"1008", "ACETOCLOR ASSA", "A"}, {"1009", "GLIFOSATO ACTIVE SIL", "R"}, {"1010", "ABRIGO PLUS", "V"}};
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (agroquimicosInformados[2].equals("R")) {
				...
				System.out.println("Ingrese el número de lote del agroquímico " + agroquimicosInformados[1].toUpperCase() + ":");
				numeroLote = new Scanner(System.in).nextLine();
				
			}
			double subTotal = capacidadEnvase * cantidadEnvases;
			ddjj.asignarDatos(agroquimicosInformados[0], agroquimicosInformados[1], capacidadEnvase, cantidadEnvases, subTotal, numeroLote);
			
			System.out.println("Si desea continuar cargando agroquímico escriba 'SI'. De lo contrario, ingrese cualquier letra.");
    ...
    private static void imprimirDDJJ(DDJJStockAgroquimico ddjj){
        ...
        System.out.println(completarConEspacios("Código Agroquímico") + "\t"+completarConEspacios("Nombre Agroquímico")+"\t"+completarConEspacios("Capacidad")+"\t"+completarConEspacios("Cantidad")+"\t"+completarConEspacios("Subtotal")+"\t"+completarConEspacios("N° de Lote")+"");
        for(DDJJStockAgroquimicoDetalle detalle : ddjj.getDetallesAgroquimicos()){
            System.out.println(completarConEspacios(detalle.getCodigoAgroquimico()) + "\t " + completarConEspacios(detalle.getNombreAgroquimico()) + " \t "
                    + completarConEspacios(Double.toString(detalle.getCapacidadEnvase())) + " \t " + completarConEspacios(Integer.toString(detalle.getCantidadEnvases())) + " \t "
                    + completarConEspacios(Double.toString(detalle.getSubTotal())) + " \t " + completarConEspacios(detalle.getNumeroLote()) + "");
        }
